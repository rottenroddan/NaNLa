...
namespace NaNLA::MemoryControllers::TransferStrategies {
    ...
    void _copyHostToHost(std::shared_ptr<r_HostMemoryController<aNumericType>> a,
                                std::shared_ptr<r_HostMemoryController<bNumericType>> b) {
    ...
    auto copyAcrossToHost(std::shared_ptr<MemoryController<NumericType>> a) -> std::shared_ptr<HostAccessible<NumericType>> {
        ...
        if(NaNLA::Internal::isCastableToTileableMemoryController(a)) {
            ...
            if(NaNLA::Internal::isTileRowMajor(std::dynamic_pointer_cast<Tileable<NumericType>>(aTile))) {
                rtnMemoryController = std::make_shared<r_TiledHostMemoryController
                        <NumericType, r_HostCacheAlignedMemoryController, RowMajorTileDetails>>
                        (aTile->getRows(), aTile->getCols(), aTile->getTileSize());
            } else {
                rtnMemoryController = std::make_shared<r_TiledHostMemoryController
                        <NumericType, r_HostCacheAlignedMemoryController, ColMajorTileDetails>>
                        (aTile->getRows(), aTile->getCols(), aTile->getTileSize());
            }
        } else {
            rtnMemoryController = std::make_shared<r_HostMemoryController<NumericType>>(a->getRows(), a->getCols());
        }
    ...
    auto copyAcrossToDevice(std::shared_ptr<MemoryController<NumericType>> a) -> std::shared_ptr<DeviceAccessible<NumericType>> {
        ...
        if(NaNLA::Internal::isCastableToTileableMemoryController(a)) {
            ...
            if(NaNLA::Internal::isTileRowMajor(std::dynamic_pointer_cast<Tileable<NumericType>>(aTile))) {
                rtnMemoryController = std::make_shared<r_TiledHostMemoryController
                        <NumericType, r_DeviceMemoryController, RowMajorTileDetails>>
                        (aTile->getRows(), aTile->getCols(), aTile->getTileSize());
            } else {
                rtnMemoryController = std::make_shared<r_TiledHostMemoryController
                        <NumericType, r_DeviceMemoryController, ColMajorTileDetails>>
                        (aTile->getRows(), aTile->getCols(), aTile->getTileSize());
            }
        } else {
            rtnMemoryController = std::make_shared<r_DeviceMemoryController<NumericType>>(a->getRows(), a->getCols());
        }
        ...
        if(NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<SrcNumericType>(srcMC) &&
           NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_HostCacheAlignedMemoryController,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
                    r_HostCacheAlignedMemoryController,
                    DstTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<SrcNumericType>>(srcMC),
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<DstNumericType, r_HostCacheAlignedMemoryController, DstTileMajorDetails>>(
                            dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_PinnedMemoryController,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
                    r_HostCacheAlignedMemoryController,
                    DstTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_PinnedMemoryController<SrcNumericType>>(srcMC),
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<DstNumericType, r_HostCacheAlignedMemoryController, DstTileMajorDetails>>(dstMC));
        } else if(NaNLA::Internal::isCastableToHostMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_HostMemoryController,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
                    r_HostCacheAlignedMemoryController,
                    DstTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_HostMemoryController<SrcNumericType>>(srcMC),
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<DstNumericType, r_HostCacheAlignedMemoryController, DstTileMajorDetails>>(dstMC));
            ...
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    ...
                    DstTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<SrcNumericType>>(srcMC),
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<DstNumericType, r_PinnedMemoryController, DstTileMajorDetails>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToPinnedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_PinnedMemoryController,
            ...
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    ...
                    DstTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<SrcNumericType>>(srcMC),
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<DstNumericType, r_HostMemoryController, DstTileMajorDetails>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_PinnedMemoryController,
        ...
        if(NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<SrcNumericType>(srcMC) &&
           NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    ...
                    r_HostCacheAlignedMemoryController,
                    SrcTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<SrcNumericType, r_HostCacheAlignedMemoryController, SrcTileMajorDetails>>(
                            srcMC),
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
                    ...
                    r_PinnedMemoryController,
                    SrcTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<SrcNumericType, r_PinnedMemoryController, SrcTileMajorDetails>>(srcMC),
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToHostMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    ...
                    r_HostMemoryController,
                    SrcTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<SrcNumericType, r_HostMemoryController, SrcTileMajorDetails>>(srcMC),
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<SrcNumericType>(srcMC) &&
                  NaNLA::Internal::isCastableToPinnedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
                    NaNLA::MemoryControllers::r_PinnedMemoryController,
                    r_HostCacheAlignedMemoryController,
                    SrcTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<SrcNumericType, r_HostCacheAlignedMemoryController, SrcTileMajorDetails>>(srcMC),
                    *std::dynamic_pointer_cast<r_PinnedMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToPinnedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
            ...
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    ...
                    SrcTileMajorDetails>(
                    *std::dynamic_pointer_cast<r_TiledHostMemoryController<SrcNumericType, r_HostCacheAlignedMemoryController, SrcTileMajorDetails>>(srcMC),
                    *std::dynamic_pointer_cast<r_HostMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    NaNLA::MemoryControllers::r_TiledHostMemoryController,
        ...
        if(NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<SrcNumericType>(srcMC) &&
           NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
            copyHostToHostResolved<SrcNumericType, DstNumericType,
                    r_HostCacheAlignedMemoryController,
                    r_HostCacheAlignedMemoryController>(
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<SrcNumericType>>(
                            srcMC),
                    *std::dynamic_pointer_cast<r_HostCacheAlignedMemoryController<DstNumericType>>(dstMC));
        } else if(NaNLA::Internal::isCastableToPinnedMemoryController<SrcNumericType>(srcMC)
                  && NaNLA::Internal::isCastableToHostCacheAlignedMemoryController<DstNumericType>(dstMC)) {
    ...
    static void r_copyHostToDeviceValues(std::shared_ptr<HostAccessible<SrcNumericType>> srcMC, std::shared_ptr<DeviceAccessible<DstNumericType>> dstMC) {
        if(NaNLA::Internal::isCastableToTileableMemoryController<DstNumericType>(dstMC)) {
                ...
                if(tileSrcMC->getTileSize() == tileDstMC->getTileSize() &&
                    ...
                    if(tileDstMC->getTileMajor() == ROW) {
                        tempMC = std::make_shared
                                <r_TiledHostMemoryController<DstNumericType, r_HostCacheAlignedMemoryController, RowMajorTileDetails>>
                                (tileDstMC->getRows(), tileDstMC->getCols(), tileDstMC->getTileSize());
                    } else {
                        tempMC = std::make_shared
                                <r_TiledHostMemoryController<DstNumericType, r_HostCacheAlignedMemoryController, ColMajorTileDetails>>
                                (tileDstMC->getRows(), tileDstMC->getCols(), tileDstMC->getTileSize());
                    }
            ...
            if(NaNLA::Internal::isCastableToTileableMemoryController<SrcNumericType>(srcMC)) {
                auto tempMC = std::make_shared<r_HostMemoryController<DstNumericType>>(srcMC->getRows(), srcMC->getCols());
    ...
    static void r_copyDeviceToHostValues(std::shared_ptr<DeviceAccessible<SrcNumericType>> srcMC, std::shared_ptr<HostAccessible<DstNumericType>> dstMC) {
        if(NaNLA::Internal::isCastableToTileableMemoryController<DstNumericType>(dstMC)) {
            ...
            if(NaNLA::Internal::isCastableToTileableMemoryController<SrcNumericType>(srcMC)) {
                ...
                if(tileSrcMC->getTileMajor() == tileDstMC->getTileMajor() &&
                    ...
                    if(tileDstMC->getTileMajor() == TileMajor::ROW) {
                        tempDstMC = std::make_shared<r_TiledDeviceMemoryController<DstNumericType, r_DeviceMemoryController, RowMajorTileDetails>>(tileDstMC->getRows(),
                                                                                                                                                  tileDstMC->getCols(),
                                                                                                                                                  tileDstMC->getTileSize());
                    } else {
                        tempDstMC = std::make_shared<r_TiledDeviceMemoryController<DstNumericType, r_DeviceMemoryController, ColMajorTileDetails>>(tileDstMC->getRows(),
                                                                                                                                                  tileDstMC->getCols(),
                                                                                                                                                  tileDstMC->getTileSize());
                    }
                ...
                if(tileDstMC->getTileMajor() == TileMajor::ROW) {
                    ...
                    tempDstMC = std::make_shared<r_TiledDeviceMemoryController<DstNumericType,
                        r_DeviceMemoryController,
            ...
            if(NaNLA::Internal::isCastableToTileableMemoryController<SrcNumericType>(srcMC)) {
                auto tempDstMC = std::make_shared<r_DeviceMemoryController<DstNumericType>>(dstMC->getRows(), dstMC->getCols());
                ...
                if(std::is_same_v<SrcNumericType, DstNumericType>) {
                    ...
                    tempSrcMC = std::make_shared<r_DeviceMemoryController<DstNumericType>>(srcMC->getRows(), srcMC->getCols());
