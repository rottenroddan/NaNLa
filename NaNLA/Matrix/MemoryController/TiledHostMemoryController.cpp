...
//    TiledHostMemoryController<NumericType, Controller, TileDetails>
//    ::TiledHostMemoryController(uint64_t rows, uint64_t cols, uint64_t tileSize,
...
//    }
    ...
    template<class NumericType, template<class> class Controller, template<class> class TileDetails>
    auto TiledHostMemoryController<NumericType, Controller, TileDetails>::at(uint64_t i, uint64_t j) -> NumericType& {
    ...
    template<class NumericType, template<class> class Controller, template<class> class TileDetails>
    auto TiledHostMemoryController<NumericType, Controller, TileDetails>::get(uint64_t i, uint64_t j) const -> NumericType  {
        if constexpr (TileDetails<NumericType>::getTileMajor() == TileMajor::ROW) {
            uint64_t index = (uint64_t)(i / this->getTileSize()) * this->getTileSize() * this->getTileSize() * this->getTileCols() +
                             (uint64_t)(j / this->getTileSize()) * this->getTileSize() * this->getTileSize() +
                             (i % this->getTileSize()) * (this->getTileSize()) +
    ...
    template<class NumericType, template<class> class Controller, template<class> class TileDetails>
    auto TiledHostMemoryController<NumericType, Controller, TileDetails>::clone() -> std::shared_ptr<MemoryController<NumericType>> {
        auto rtnMemoryController = std::make_shared<TiledHostMemoryController<NumericType, Controller, TileDetails>>
