...
#include <NaNLA/Matrix/MemoryController/TiledDeviceMemoryController.h>
...
TEST(TEST_SUITE_NAME, ShouldBeAbleToCloneSelf) {
    ...
    auto tdmc = std::make_shared<TiledDeviceMemoryController<float, DeviceMemoryController, RowMajorTileDetails>>(256, 256, 16);
    NaNLA::MemoryControllers::TransferStrategies::copyValues<float, float>(hmc, tdmc);
...
TEST(TEST_SUITE_NAME, ShouldBeAbleToCopyConstruct) {
    ...
    auto hmc = std::make_shared<HostMemoryController<float>>(256, 256);
    NaNLA::Test::Utilities::populateHMCWithRandomValues<float>(hmc);
    auto dmc1 = std::make_shared<TiledDeviceMemoryController<float, DeviceMemoryController, RowMajorTileDetails>>(256, 256, 16);
    NaNLA::MemoryControllers::TransferStrategies::copyValues<float, float>(hmc, dmc1);

    auto dmc2 = std::make_shared<TiledDeviceMemoryController<float, DeviceMemoryController, RowMajorTileDetails>>(*dmc1.get());
    auto testHmc = std::make_shared<HostMemoryController<float>>(256, 256);
    ...
    auto dmc3 = std::make_shared<TiledDeviceMemoryController<float, DeviceMemoryController, RowMajorTileDetails>>(*dmc1.get());
